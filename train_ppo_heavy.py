#!/usr/bin/env python3
"""Train PPO agent on heavier traffic load (α=1.0).

Heavier traffic should give agent more opportunity to optimize routing
since OSPF may be suboptimal under congestion.

Usage:
    CUDA_VISIBLE_DEVICES=1 python train_ppo_heavy.py
"""
import os
...
print("=" * 80)
print("PPO Training for Network Routing - HEAVY LOAD (α=1.0)")
print("=" * 80)

# Environment setup
print("\n[1/5] Creating gym environment (α=1.0 moderate load)...")
env = RoutingEnv(
    topo_name="abilene",
    traffic_file=THESIS_DIR / "results" / "traffic_abilene_α1.0_min30.json",
...
print(f"  ✓ Traffic load: α=1.0 (moderate, ~10.9% OSPF baseline expected)")
...
checkpoint_callback = CheckpointCallback(
    ...
    name_prefix="ppo_routing_heavy",
    save_replay_buffer=False,
)
print(f"  ✓ Checkpoints: {MODELS_DIR}")

# Training
print("\n[4/5] Training for 50,000 timesteps (~833 episodes, heavy load)...")
print("      Expected: Agent should learn to optimize under congestion...")
print()
...
try:
    ...
    traceback.print_exc()
    try:
        partial_model_path = MODELS_DIR / "ppo_routing_heavy_partial"
...
final_model_path = MODELS_DIR / "ppo_routing_heavy_final"
model.save(final_model_path)
print(f"  ✓ Final model saved to: {final_model_path}")

# Quick evaluation
print("\n[Bonus] Quick evaluation (5 episodes on heavy load)...")
env.close()
eval_env = RoutingEnv(
    topo_name="abilene",
    traffic_file=THESIS_DIR / "results" / "traffic_abilene_α1.0_min30.json",
...
print(f"  - Heavy load model (α=1.0): {final_model_path}.zip")
print(f"  - Comparison: Check mean max_util above vs OSPF 10.9% baseline")
print()
