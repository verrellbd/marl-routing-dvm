...
The policy was trained on seventeen SNDLib topologies across three different seeds, then evaluated on three new topologies. Details about training convergence are in Appendix ~\ref{app:training} and Table ~\ref{tab:reward-decomp} presents a comparison of the trade-offs among congestion, delay, and hop counts. On Abilene the policy detours on only $3.0\%$ of hops for a delay cost of $4.8$ and reduces the bottleneck to $96\%$ of OSPF's, and on GÉANT it detours on $3.8\%$ of hops for a cost of $8.4$ and reaches $67\%$; in both, the congestion term dominates the return and the delay penalty is small. Germany50 shows the largest improvement, to $47\%$ of OSPF's bottleneck, but there the delay penalty of $61.0$ is the larger of the two terms, bought with detours on $14.2\%$ of hops, and the overall return falls below the $-100$ that OSPF scores by construction. Therefore, the trade-off of the policies for unseen topologies results in larger topologies paying more in detours than they recover in congestion.
...
Figure~\ref{fig:link-utilization} breaks the offered-load panel of Figure~\ref{fig:heatmap} out by topology, with the underlying values listed in Table~\ref{tab:offered-util}. The results show that MARL lowers the offered load in both regimes on two of the three networks. MARL achieves superior performance compared to OSPF, ECMP, and single-agent approaches on the GEANT and Germany50 networks, although it performs slightly worse than the single-agent method on Abilene. Additionally, MARL demonstrates a significant reduction in maximum offered load, achieving up to a 53\% decrease on GEANT and Germany50 in both regimes. In the overload regime, MARL reduces the offered load to 96.6\%, maintaining it below the 100\% threshold.

In the feasible regime, OSPF reaches a maximum offered load of 81\% on Germany50, while ECMP achieves 70.4\%. ECMP works more efficiently than OSPF on both GEANT and Germany50, reducing the maximum offered load by about 11\%. On the other hand, single-agent shows significant improvement on Germany50 to 52\%, but underperforms on GEANT at 70\%. MARL has better performance than all three baselines on GEANT and Germany50, achieving 48.2\% and 40.9\%, respectively.

The overload regime produces similar results to the feasible regime. In this setting, both the single-agent and MARL methods improve routing decision performance. The single-agent approach beats OSPF and ECMP on Abilene and Germany50, with improvements of 10\% and 29\%. MARL also shows strong results, reducing the maximum offered load on GEANT and Germany50 to 96.6\% and 117\%, respectively.
...
These differences are statistically supported. In the feasible regime MARL lowers the maximum link utilization on all nine paired matrices, by $17.2$ percentage points on average (Wilcoxon signed-rank, $W=0$, $p=0.004$). In the overload regime it lowers packet loss on six of the nine, by $8.9$ points on average, but the rank test does not reach the conventional threshold ($W=6$, $p=0.055$); the three matrices that favour OSPF are the three Abilene matrices. Restricted to GÉANT and Germany50 the improvement holds on all six matrices ($W=0$, $p=0.031$). The headroom result is therefore significant across every network tested, whereas the loss result is significant only where the decentralized formulation works at all.
