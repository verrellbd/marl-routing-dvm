...
\tfrac{1}{100}\!\max_{e\in\mathrm{out}(i)}\!(100{-}u_e),\;
\tfrac{\deg(i)}{\Delta}\,\bigr]\in\mathbb{R}^{6},
\end{equation}
where $c_t$ is the node holding the flow, $h_{\max}$ the largest OSPF-cost distance
to $d_f$ over the network, and $\Delta=60$ a fixed padding bound on degree, chosen to
exceed the largest degree in any training or evaluation topology so that one
parameter set covers them all. The fourth and fifth entries report the worst
congestion and the best headroom among the links leaving $i$, so they describe
different links and neither determines the other.
...
\end{equation}
where $\ell_{c_tj}\in\mathbb{R}^{4}$ represents the utilization of the link to $j$, the bottleneck that would result from placing the current flow on that link, the headroom remaining on it, and the normalized distance from node $j$ to the destination. The variable m indicates the mask defined in \eqref{eq:mask}. As a result, unacceptable hops are assigned zero probability. This approach ensures every sampled action avoids loops by design. Since $\mathrm{MLP}_\theta$ is used for each neighbor and shared across all neighbors, the policy works for any node degree. On the other hand, an output layer matched to the action set would not generalize across different topologies.
