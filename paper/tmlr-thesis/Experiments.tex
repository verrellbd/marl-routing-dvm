...
The experiment was conducted on a single Linux server (Rocky 9) equipped with an AMD EPYC 7543 processor (128 hardware threads) and 512 GB of RAM. Model training and evaluation utilized CPU-based concurrent core parallelization. The software stack included Python 3.9, ns-3.42, PyTorch 2.6.0, PyTorch Geometric 2.6.1, NetworkX, and Stable-Baselines3 2.7.1. The pipeline was implemented using several versions of Python modules. To facilitate reproducibility in other environments, we provide a README file (\texttt{readme.md}) and a Jupyter notebook (\texttt{experiment.ipynb}).

\subsection{Networks and Traffic Resources}
\label{sec:setup-instances}

The network simulation utilizes a set of topologies and traffic patterns. Seventeen topologies from SNDLib, excluding Abilene, GEANT, and Germany50, are used to train the model. The training set includes topologies ranging from 10 to 54 nodes and 21 to 80 edges, which enhances the model's ability to generalize across different network structures. In contrast, traffic data require a different approach because measured traffic is only available for Abilene, GEANT, and Germany50.

To address the limitations of SNDLib resources for policy training, synthesized traffic is generated using the TMgen library and the modulated-gravity model \cite{internet-traffic}. This model represents inter-node traffic by analogy with Newton's law of gravitation and serves as the standard generator for such purposes. For each topology, traffic patterns are generated and evaluated across five volume scales $\{0.6, 0.8, 1.0, 1.2, 1.5\}$, with normalization such that scale $1.0$ sets the OSPF bottleneck at exactly $100\%$. This methodology enables the evaluation of both feasible and congested traffic regimes.

Additionally, the measured traffic in SNDLib is insufficient to utilize the capacity of the backbone topologies fully. To simulate scenarios where link capacities are exceeded, demand scaling variations are applied to the evaluation topologies.  The three backbones span $12$, $22$ and $50$ nodes. Abilene is the only one with  eterogeneous capacities, fourteen links at $9.92$\,Gb/s and one at $2.48$; GEANT and Germany50 are uniform at $40$\,Gb/s. Because a correctly costed OSPF is never overloaded on Abilene at the analytical demand scales and never feasible on Germany50 at the packet-level ones, those two networks are measured at two scale sets each, so their rows in the two result tables describe the same policies at different operating points and must not be read across. Full instance properties and the complete evaluation protocol are given in Table~\ref{tab:eval-setup} of Appendix~\ref{app:instances}.
...
\begin{enumerate}
    \item \textbf{OSPF.} The OSPF cost metric is defined as $\text{cost}(e) = \text{refBW}/\text{cap}(e)$. This configuration is commonly deployed in practice and provides a more robust baseline than simple hop-count metrics, particularly in networks with heterogeneous link capacities.
...
Table~\ref{tab:hyper} presents the learner configurations. Every optimizer hyperparameter is held equal across the two architectures, so any difference in outcome is attributable to decentralization rather than to tuning. The centralized controller keeps the default settings of Stable-Baselines3 \cite{stable-baselines3}, and MAPPO is adapted to these defaults rather than modifying the baseline to match the proposed method. This approach avoids disadvantaging the baseline. Both methods therefore perform 732 updates over $1.5\times10^{6}$ environment steps using three random seeds.
...
\hline
\end{tabular}
\caption{Hypermaraters}
\label{tab:hyper}
\end{table}


The two policies differ in capacity due to architectural differences, resulting in unequal hidden widths. The centralized policy contains $268{,}420$ trainable parameters, whereas the decentralized policy contains $17{,}570$. Consequently, the centralized controller possesses approximately fifteen times the capacity of the decentralized method. Equalizing parameter counts would require distorting one of the architectures. The decentralized width was selected based on performance on training topologies, not on evaluation backbones. Widths were evaluated using held-out TMgen matrices from seventeen training instances, generated with a seed distinct from that used in training.
...
This quantity does not impose an upper bound. It is computed from the demands alone, and therefore can exceed $100\%$. This approach permits demand to surpass capacity, enabling the expression of routing pressure on the network. The offered load is evaluated using an analytical surrogate to assign regime labels to each traffic matrix. Each test matrix is categorized based on the offered utilization that \emph{OSPF} would produce: \textbf{overload} when $U_{\mathrm{OSPF}}\ge100\%$, and \textbf{feasible} when $U_{\mathrm{OSPF}}<100\%$.

To validate the proposed method, the offered load is evaluated using ns-3 as the primary network performance simulator. Results show that, when the overload matrix is applied with OSPF in ns-3, packet loss ranges from approximately 14 to 27 percent. In contrast, for feasible matrices, packet loss is between 0.03 and 0.17 percent. Utilization in the ns-3 simulations is limited to 100 percent. These results demonstrate that the traffic matrix labeling method is effective and appropriate for subsequent network performance evaluations.

\subsection{Packet-Level Evaluation Configuration}
\label{sec:setup-ns3}

Following Section~\ref{sec:setup-regimes}, the evaluation uses ns-3 to simulate labeled traffic. Each link is a \texttt{PointToPointNetDevice} with the default DropTail transmit queue of 100 packets, and no active queue management, shaping, or congestion control is enabled, so all reported packet loss results solely from buffer overflow. Traffic is one-way UDP, each demand a constant-rate \texttt{OnOff} application with a 1400-byte payload, so loss and delay follow from the routing decision rather than from transport-layer feedback. Propagation delays are installed at nanosecond resolution, which matters on Germany50 where several links fall below 1 ms. Each run spans 8 seconds of simulated time with flows active over [2 s, 9 s], and all utilization figures are divided by the uniform factor 7/8 to correct for that difference. Demand rates and link capacities are both divided by 20, which preserves every utilization, loss, and delay ratio while keeping the packet count tractable. The complete grid consists of 378 simulations, all of which completed successfully.
