...
% results/final_ns3_grid.json (558 ns-3 simulations, 0 failures, dirs results/ns3f_*).
% Launchers: train_seeds10.sh (training), eval_seeds10.sh (evaluation).
% Aggregators: rebuild_ns3_grid.py, fill_offered_grid.py, decompose_reward.py.
...
This study uses ns-3 to evaluate labeled traffic. Each link is a \texttt{PointToPointNetDevice} with the default DropTail transmit queue of 100 packets, and no active queue management, shaping, or congestion control is enabled, so all reported packet loss results solely from buffer overflow. Traffic is one-way UDP, each demand a constant-rate \texttt{OnOff} application with a 1400-byte payload, so loss and delay follow from the routing decision rather than from transport-layer feedback. Propagation delays are installed at nanosecond resolution, which matters on Germany50 where several links fall below 1 ms. Each run spans 9 seconds of simulated time with flows active from 2 seconds until the end of the run, while utilization is accumulated over an 8-second window. All utilization figures are adjusted by dividing by 7/8 to account for the 7 seconds of activity within that window. Demand rates and link capacities are both divided by 20, which preserves every utilization, loss, and delay ratio while making the packet count manageable. Last, the complete grid consists of 558 simulations: three routing arms across ten model seeds on eighteen evaluation matrices, together with the eighteen deterministic OSPF runs.
...
Differences between MARL and OSPF are tested with a two-sided Wilcoxon signed-rank test paired on the traffic matrix, because each learned routing is simulated on exactly the same matrix as the OSPF routing it is compared against, so only the routing varies. The ten model seeds are averaged before pairing, since they are ten draws of a single policy rather than ten independent observations of a matrix. Each regime therefore contributes nine paired matrices, three from each backbone.
