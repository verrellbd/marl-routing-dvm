...
\begin{enumerate}
    ...
    \item \textbf{ECMP.} Equal-Cost Multipath \citep{hopps2000ecmp} over the paths that the same cost metric leaves tied, assigned one path per flow by hashing the packet header rather than divided fractionally. Because the tie is on cost, ECMP inherits OSPF's capacity awareness and forms a stronger baseline than OSPF alone on the larger networks.
